...
import { isDirectAccessExpired } from "./student-direct-access.js";
...
const createSessionForStudent = (student, options = {}) => {
  const token = makeRandomToken();
  const nowIso = getNowIso();
  const defaultExpiresAt = addDays(SESSION_DAYS);
  const requestedExpiresAtMs = Date.parse(String(options.expiresAt || ""));
  const defaultExpiresAtMs = Date.parse(defaultExpiresAt);
  const expiresAt = Number.isFinite(requestedExpiresAtMs)
    ? new Date(Math.min(defaultExpiresAtMs, requestedExpiresAtMs)).toISOString()
    : defaultExpiresAt;
...
const findStudentBySession = (students = [], req) => {
  ...
  for (let index = 0; index < students.length; index += 1) {
    ...
    if (isDirectAccessExpired(student, now)) continue;
    const auth = getStudentAuth(student);
