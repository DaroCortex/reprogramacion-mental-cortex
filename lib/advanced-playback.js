...
export const getAdvancedVoiceTargetVolume = ({
  ...
  if (phase === "apnea" && hasExplicitVolume(apneaVolume)) {
    return clampVolume(apneaVolume);
  }
  if (
    (phase === "breathing" || phase === "recovery") &&
    hasExplicitVolume(breathingVolume)
  ) {
    return clampVolume(breathingVolume);
  }
